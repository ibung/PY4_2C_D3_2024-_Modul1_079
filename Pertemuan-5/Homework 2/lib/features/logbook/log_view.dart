...
class _LogViewState extends State<LogView> {
  ...
  late String _currentTeamId; // ← TAMBAH
  ...
  void initState() {
    ...
    _currentTeamId   = widget.teamId; // ← TAMBAH

    _logController = LogController()
      ..currentUserId   = _currentUserId
      ..currentUserRole = _currentUserRole
      ..currentTeamId   = _currentTeamId; // ← TAMBAH

    // Pass teamId yang benar, bukan string kosong
    _logController.startConnectivityListener(_currentTeamId);
    _loadLogs();
  }
  ...
  Future<List<Logbook>> _fetchWithConnectionGuard() async {
    ...
    // Pass teamId yang benar saat sync ke Hive
    await _logController.loadLogs(_currentTeamId); // ← FIX: bukan ''
                              ...
                              onDismissed: (_) async {
                                ...
                                if (!AccessControlService.canPerform(
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text(
                                          'Anda tidak punya izin untuk menghapus catatan ini'),
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Lottie.asset(
            'assets/json/Sample Animation.json',
            width: 180,
            height: 180,
            fit: BoxFit.contain,
          ),
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => LogEditorPage(
          ...
          currentUserId: _currentUserId,
          currentUserRole: _currentUserRole,
          currentTeamId: _currentTeamId, // ← FIX: bukan ''
        ),
      ),
    ).then((_) => _loadLogs());
  }
  ...
  void _confirmDelete(Logbook log) {
    ...
    if (!AccessControlService.canPerform(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Anda tidak punya izin untuk menghapus catatan ini'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    return Card(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(
          ...
          child:
              const Icon(Icons.article, color: Color(0xFF1E3A5F)),
        ),
        title: Row(
          children: [
            Expanded(
